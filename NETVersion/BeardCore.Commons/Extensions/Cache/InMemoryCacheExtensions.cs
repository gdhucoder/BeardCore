...
namespace BeardCore.Commons.Extensions.Cache
{
    public static class InMemoryCacheExtensions
    {
        ...
        /// <returns></returns>
        private static MemoryCacheEntryOptions SetOptions(TimeSpan? absoluteExpireTime, TimeSpan? unusedExpireTime)
        {
        ...
        public static T GetRecord<T>(this IMemoryCache cache, string recordId)
        {
            ...
            if (!cache.TryGetValue(recordId, out string jsonData))
            {
                return default(T);
            }
